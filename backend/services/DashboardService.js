...
export class DashboardService {
  ...
  static async getNumberOfCustomers() {
    const query = 'SELECT COUNT(DISTINCT customerId) AS distinct_customer_count FROM customer;';
    return await connection.promise().query(query);
  }
  catch(error) {
  ...
  static async getNumberOfAccessories() {
    const query = 'SELECT COUNT(DISTINCT accessoryId) AS distinct_accessory_count FROM accessory;';
    return await connection.promise().query(query);
  }
  catch(error) {
  ...
  static async accessoryVsStock() {
    const query =
      'SELECT a.accessoryId, SUM(s.quantity) AS totalQuantity FROM accessory a JOIN stock s ON a.accessoryId = s.accessoryId GROUP BY a.accessoryId;';
    return await connection.promise().query(query);
  }
  catch(error) {
