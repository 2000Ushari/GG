...
function Payment({ paymentDetails }) {
  ...
  const getCustomerDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/api/customer/getCustomerDetails/${paymentDetails.userId}`
      );
  ...
  const handlePayment = async () => {
    try {
      // Request backend to generate the hash value
      const response = await fetch('http://localhost:3001/api/payment/getHash', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <Box>
      <Button id="payhere-payment" variant="contained" color="success" onClick={handlePayment}>
        Pay Now!
      </Button>
    </Box>
  );
